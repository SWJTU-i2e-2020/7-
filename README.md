...
### 可替代概念的产生
1.可携带性：灵巧轻便，便于携带，携带舒适。    
2.准确性：数据传输应当准确，减少误报发生。   
3.即时性：数据传输延迟要尽量低，保证在最短时间完成求救到接收的过程。  
4.警报响度：发出的警报声音要清晰响亮，能让人立刻注意到，并且要有一定持续性。  
5.续航性：尽量选择绿色环保的供电形式，例如：充电器，太阳能，移动电源等。续航时间要有所保证。  
6.产品自身的保护措施：有封闭的外壳，能够防水防尘，有一定的抗摔抗撞击的能力。  
7.传感数据的广度：实现多种身体数据并行传输，由产品自身综合判断。  
8.辅助提示：警报发生后，显示相对位置。  
![外观](https://github.com/SWJTU-i2e-2020/7-Outdoor-travel/blob/main/image/%E5%A4%96%E8%A7%82.jpg)
### 概念选择
1.可携带性：在容纳各种模块的情况下缩小产品体积，采用轻质材料封装。  
2.准确性与即时性：采用ESP开发板结合Lora模块通信技术实现信息传递，用按钮按下即视为发出求救，求救过程时间极短。  
3.警报响度：使用TMB12A12型蜂鸣器发出报警，并新增可与手机等移动设备相连接功能，报警即可收到提示信息，多方提醒。  
4.续航性：采用移动电源供电的方式，减少产品电耗，延长续航时间。  
5.自身保护：产品完全封闭，内置保护层，予以保护。  
实现功能：自主手动报警，体温监测自动报警。  
## 五。详细设计
### 物料清单
| 序号 | 名称 | 规格/型号 | 封装 | 用量 | 品牌 | 商品编号 |
|:---|:---|:---|:---|:---|:---|:---|
...
### 实现功能
本产品主要实现自助SOS求救信号发送功能，以及接收端接收发射端使用者体温数据并自行判断是否发出求救警报功能。发出警报的方式是通过云端接收信息和蜂鸣器报警同步进行。
### 制作过程
1.分别对网关和节点进行ESP32开发环境搭建。  
2.将贴片Lora RA-02SX1278焊接在模块和扩展板上。  
3.焊接上排针。  
4.根据程序设计连接Lora和ESP32模块电路。  
5.用arduino给ESP32写入已经调试好的程序，节点与网关程序不同。  
6.在面包板上连接按钮，并串联入电阻进行保护。  
7.发射端连接温度传感器。  
8.分别封装两块开发板，发射端预留存放移动电源空间，接收端可完全封闭。  
9.将接收端与移动数据信/Wifi号相连,即可开始测试使用。  
## 六.性能指标
| 内容 | 参数 |
|:---|:---|
|供电方式|200W移动电源临时供电|
...
4)每个项目成员都有个人安全，情感交流，尊重和自我价值实现的需要。加强项目团队管理。创造竞争，公平和发展的气氛与环境，有利于团队成员努力工作，证明自己的价值，增强其成就感，在团队中使成员相互关心，相互帮助，相互尊重。  
  ![外观](https://github.com/SWJTU-i2e-2020/7-Outdoor-travel/blob/main/%E6%A6%82%E5%BF%B5%E8%8D%89%E5%9B%BE.jpg)    
  ![内部](https://github.com/SWJTU-i2e-2020/7-Outdoor-travel/blob/main/image/%E5%86%85%E9%83%A8.jpg)
